...
export const useStaking = (
  ...
  const { data: valAllAddresses } = useValidatorConsumerProviderAddressesQuery();
  ...
  useEffect(() => {
    ...
    if (valAllAddresses) {
      if (providerAddress.startsWith(prefix.validator)) {
        const matchingValidator = valAllAddresses.ccv_validator.find(
          (x) => x.consumerOperatorAddress === providerAddress
        );
        if (matchingValidator) {
          provider = matchingValidator.providerSelfDelegateAddress ?? '';
        } else {
          provider = address;
        }
  ...
  }, [address, providerAddress, valAddressesInfo, valAllAddresses]);
