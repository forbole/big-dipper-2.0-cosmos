...
const formatGov = (data: ParamsQuery) => {
  if (data.govParams.length) {
    ...
    return {
      ...
      default: {
        quorum:
          numeral(numeral(govParamsRaw.tallyParams.default.quorum).format('0.[00]')).value() ?? 0,
        threshold:
          numeral(numeral(govParamsRaw.tallyParams.default.threshold).format('0.[00]')).value() ??
          0,
        vetoThreshold:
          numeral(
