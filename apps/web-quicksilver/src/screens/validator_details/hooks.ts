...
const formatOverview = (data: ValidatorDetailsQuery) => {
  ...
  const consensusAddress = data?.validator?.[0]?.validatorInfo?.consensusAddress ?? '';
  const profile = {
...
export const useValidatorDetails = () => {
  ...
  // Celestia Profile
  // ==========================
  const { profile: dataDesmosProfile, loading: loadingDesmosProfile } = useConsensusProfile(
    state.overview.consensusAddress,
    state.overview.selfDelegateAddress
  );
  useEffect(() => {
    if (dataDesmosProfile) {
      const { name, imageUrl, bio } = dataDesmosProfile;
      setState((prevState) => ({
        ...prevState,
        desmosProfile: {
          name,
